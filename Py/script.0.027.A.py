...
target_cols_i = ['ind_ahor_fin_ult1', 'ind_aval_fin_ult1', 'ind_cco_fin_ult1', 'ind_cder_fin_ult1', 'ind_cno_fin_ult1',
...
def cvXGB(train_X, train_Y, seed_val=123, param={}, num_rounds=110, nfold=10, cvseed=100,verbose_eval=0):
    plst = list(param.items())
    xgtrain = xgb.DMatrix(train_X, train_Y)
    evalh = xgb.cv(plst, xgtrain, num_boost_round=num_rounds, nfold=nfold, verbose_eval=verbose_eval,seed=cvseed)
    return evalh


def runXGB(train_X, train_y, seed_val=123, param={}, num_rounds=110):
    plst = list(param.items())
    xgtrain = xgb.DMatrix(train_X, label=train_y)
    evalHistory = xgb.train(plst, xgtrain, num_rounds)
    return evalHistory
